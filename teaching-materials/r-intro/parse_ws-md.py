...
# Setting paths
notebooks_path = "./notebooks/convert/"
...
chapter_text_regex = re.compile('(?<=# {#Kapitel}).*?(?=\n# {#Afsnit}|\n# {#Kapitel}|\n# {#END})', re.DOTALL)
...
def create_section_filename(title):
    filename = re.findall(r'^.*?(?=\s|$)', title)[0].lower()
...
for chaptkey,chapter in chapters_dict.items():
    ...
    for sectkey,section in chapter['sections'].items():
        ...
        with open(join(folder, filename + '.md'), 'w') as outfile:
